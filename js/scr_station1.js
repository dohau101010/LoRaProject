...
var rainChart = [10, 13, 14, 8, 4];
var temptChart = [28, 29, 30, 35, 37, 40, 38, 35, 35, 28];
var humidChart = [90, 88, 87, 86, 84, 80, 75, 70, 70, 65];
...
database.ref("/Trạm 1/Nồng độ CO").on("value", function (snapshot) {
...
database.ref("/Trạm 1/Bụi").on("value", function (snapshot) {
  var rain = snapshot.val();
  document.getElementById("rain").innerHTML = rain;
});
...
database.ref("/Trạm 1/Nhiệt độ").on("value", function (snapshot) {
  var tempt = snapshot.val();
  document.getElementById("tempt").innerHTML = tempt;
  database.ref("/Ngưỡng 1/Nhiệt độ").on("value", function (snapshot) {
    ...
    if (tempt > tempt_th && !hasPushedToHistory) {
      ...
      firebase.database().ref("/History 1").set({
        mess: "Nhiệt độ cao!",
      });
...
database.ref("/Trạm 1/Độ ẩm không khí").on("value", function (snapshot) {
  var humi = snapshot.val();
  document.getElementById("humi").innerHTML = humi;
  database.ref("/Ngưỡng 1/Độ ẩm không khí").on("value", function (snapshot) {
